# -*- coding: utf-8 -*-
"""Create the full slide version, step 1 (read-only): inventory the taped
video decks in `Recorded Video Slides/` and pair every taped slide with a
slide of `Module X - Revised.pptx` by content fingerprint (never by number).

Module-agnostic since 2026-09-24: the module folder is the script's own
folder (move it to `_build_files/` and set MODULE = HERE.parent), and both
the Revised deck and the taped decks are found by glob.
...
REVISED = next(iter(sorted(MODULE.glob("Module * - Revised.pptx"))))
...
def main():
    rev = deck(REVISED)
    vdecks = sorted(p for p in VDIR.glob("Module * - Video *.pptx"))
