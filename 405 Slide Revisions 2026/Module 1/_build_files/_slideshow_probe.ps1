...
param([int[]]$Slides = @(1, 12, 33, 34, 40, 47, 52, 54, 57, 70, 86, 87, 91, 92),
      [string]$Deck = "Module 1 - Final.pptx")
$folder = Split-Path $PSScriptRoot -Parent   # _build_files/ since 2026-09-24
$deck = Join-Path $folder $Deck
if (-not (Test-Path $deck)) { $deck = Join-Path (Split-Path $folder -Parent) $Deck }
