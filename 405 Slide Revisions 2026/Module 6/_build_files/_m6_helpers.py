...
def _add_convention_box(slide, left, top, width, height, *,
                          ...
                          fill_rgb=None, fill_alpha=None, text_color=None,
    ...
    def _style_run(r, opts):
        ...
        # `text_color` lets a card carry a dark fill and white text
        # (2026-09-14): the three-degrees cards are red / navy / cream
        r.font.color.rgb = opts.get('color', text_color or NAVY)
    ...
    if runs is not None:
        ...
        for entry in runs:
            ...
            if opts.get('bullet'):
                ...
                # the glyph is selectable: slide 34 marks the operative
                # half of the rule with a double arrow rather than a dot
                # (2026-09-14, Nico)
                _bchar = opts.get('bullet_char', '\u2022')
                _bfont = ('Cambria Math' if _bchar != '\u2022' else 'Arial')
                for tag, attrs in (
                        ('a:buFont', {'typeface': _bfont,
                                      'panose': '020B0604020202020204',
                                      'pitchFamily': '34', 'charset': '0'}),
                        ('a:buChar', {'char': _bchar})):
        ...
        p = tf.paragraphs[0]
        p.alignment = align
        # ``text_color`` has to reach THIS path too (2026-09-14): the
        # prefix/body shortcut was passing an explicit NAVY, which beat the
        # argument and left slide 12's navy card with navy text on it --
        # invisible, and the red card unreadable.
        tc = text_color or NAVY
        if prefix:
            r1 = p.add_run(); r1.text = prefix
            _style_run(r1, {'bold': True, 'color': tc, 'size': size})
        if body:
            r2 = p.add_run(); r2.text = body
            _style_run(r2, {'color': tc, 'size': size})
...
def _add_media_image(slide, fname, *, left, top, width=None, height=None,
                     rounded=True, shadow=True, corner_pct=8, crop=None):
    """Place a source-deck image by filename from _source_images/.
    Logos / screenshots / clippings: rounded=False, shadow=False.

    ``crop`` is ``(left, right, top, bottom)`` as fractions of the source
    image, the way PowerPoint stores one.  Nico's own slides use a crop to
    show ONE wide photo as two stacked halves (his slide 21), so the
    rebuild needs it to reproduce that rather than placing the photo whole.
    """
...
def _fig_axes(slide, fig, *, weight_pt=2.0,
    ...
    The four pieces are NAMED (2026-09-15).  "A chart's axes and axis
    labels are visible from the start and are never animated" is an old
    rule, but nothing on the shape said which boxes those were, so the
    animation engine had no way to tell an axis title from a tick label
    and the auto-rollout gave "P" and "Q" clicks of their own on seven
    slides.  Naming them is the same device the curve, region and leader
    pairings already use -- an exact key beats a geometric guess.
    """
    _ax_y = _add_arrow(slide, (fig.x(0), fig.y(0)),
                       (fig.x(0), Inches(fig.b - fig.h - 0.18)),
                       color=NAVY, weight_pt=weight_pt, head=True)
    _ax_x = _add_arrow(slide, (fig.x(0), fig.y(0)),
    ...
    if y_title:
        if titles_at_tip:
            ...
            _t = _add_text(slide, Inches(fig.l - w - Y_TITLE_GAP),
                           Inches(tip_y - 0.145),
                           Inches(w), Inches(0.29), y_title,
                           size=label_size, bold=True, italic=True,
                           color=NAVY, font="Calibri")
        else:
            _t = _add_text(slide, Inches(fig.l - 0.75),
                           Inches(fig.b - fig.h - 0.62),
                           Inches(2.0), Inches(0.32), y_title,
                           size=label_size, bold=True, italic=True,
                           color=NAVY, font="Calibri")
        _t.name = "sdaxistitle:y"
    if x_title:
        if titles_at_tip:
            ...
            _t = _add_text(slide, Inches(tip_x - w / 2.0),
                           Inches(fig.b + 0.05),
                           Inches(w), Inches(0.29), x_title,
                           size=label_size, bold=True, italic=True,
                           color=NAVY, font="Calibri")
        else:
            _t = _add_text(slide, Inches(fig.l + fig.w - 0.4),
                           Inches(fig.b + 0.10),
                           Inches(1.8), Inches(0.32), x_title,
                           size=label_size, bold=True, italic=True,
                           color=NAVY, font="Calibri")
        _t.name = "sdaxistitle:x"


def _fig_line(slide, fig, p0, p1, *, color=NAVY, weight_pt=2.5, dash=None,
              head=False, curve=None):
    """Straight curve segment in logical coords.

    ``curve="D"`` stamps the line "sdcurve:D" so the grouping pass can pair
    it with the label that names it (2026-09-14, Nico: "everywhere in
    Module 6, group a curve with its label").  Pairing by NAME, not by
    proximity: a chart has several lines and several labels, and the
    nearest label to a line is regularly the wrong one.
    """
    ln = _add_arrow(slide, (fig.x(p0[0]), fig.y(p0[1])),
                    (fig.x(p1[0]), fig.y(p1[1])),
                    color=color, weight_pt=weight_pt, dash=dash, head=head)
    if curve and ln is not None:
        ln.name = "sdcurve:%s" % curve
    return ln


def _poly_key(pts):
    """A stable id for a region polygon, from its own coordinates.

    `_fig_poly` stamps it on the region and `_region_label` stamps it on
    the label, so the grouping pass can pair the two EXACTLY instead of
    guessing from geometry -- which grouped curve labels ("D", "MR") with
    whatever shaded region they happened to sit inside (2026-09-10).
    """
    import zlib
    raw = "|".join("%.4f,%.4f" % (float(x), float(y)) for x, y in pts)
    return "%08x" % (zlib.crc32(raw.encode("utf-8")) & 0xFFFFFFFF)


def _fig_poly(slide, fig, pts, *, fill=CREAM, line=NAVY, alpha=None,
              line_w=1.25):
    """A filled polygon in logical coordinates — surplus triangles, revenue
    rectangles, block-pricing steps, profit / loss regions.

    Built as a custGeom injected into a RECTANGLE autoshape, so the region
    stays editable ("Edit Points" shows one handle per vertex).

    Lifted verbatim from ``_fig_poly`` in Module 4/_build_Module4.py, which
    is the proven implementation.  Do NOT reach for
    ``shapes.build_freeform().convert_to_shape()`` instead: python-pptx
    accepts it and saves happily, but PowerPoint then refuses to open the
    file at all ("PowerPoint could not open the file") — found on the
    Module 6 Netflix surplus slide, 2026-09-09.
    """
    ...
    if alpha is not None:
        sf = spPr.find(qn('a:solidFill'))
        sf.find(qn('a:srgbClr')).append(ET.fromstring(
            '<a:alpha xmlns:a="%s" val="%d"/>' % (A_NS, int(alpha))))
    shp.name = "sdregion:%s" % _poly_key(pts)
    return shp
...
_SWATCH_GEOM = {"sq": (MSO_SHAPE.RECTANGLE, False),
                ...
                # a legend line for a CURVE, not an area: a
                # short bar in the curve's own colour, drawn
                # solid rather than washed, because a 2 pt line
                # on the chart is solid too (2026-09-10)
                "line": (MSO_SHAPE.RECTANGLE, False)}
# Which way the flip goes.  MSO_SHAPE.RIGHT_TRIANGLE draws its right
# angle at the bottom LEFT... in the legend it came out mirrored
# against the surplus triangle on Module 6 slide 49, whose vertical
# leg is the y axis, so "tri_h" mirrors it back (2026-09-10).
_SWATCH_FLIP = {"tri_v": "flipV", "tri_h": "flipH"}
_SWATCH_GEOM["tri_h"] = (MSO_SHAPE.RIGHT_TRIANGLE, True)
...
def _welfare_rows(slide, left, top, width, rows, *, pitch=0.46,
    ...
    for row in rows:
        ...
        for j, mark in enumerate(marks):
            # a mark may carry its OWN wash: one legend can now name a
            # 26 % gold area and a 16 % navy one, and each swatch has to
            # match the area it stands for (2026-09-15)
            if isinstance(mark, tuple) and len(mark) == 3:
                col, kind, mark_alpha = mark
            elif isinstance(mark, tuple):
                (col, kind), mark_alpha = mark, alpha
            else:
                col, kind, mark_alpha = mark, "sq", alpha
            geom, flip = _SWATCH_GEOM[kind]
            dx = j * step if layout == "h" else 0.0
            dy = 0.0 if layout == "h" else j * (swatch + 0.01)
            # a "line" mark is a thin bar centred on the row, so
            # it reads as a line and not as a squashed area
            mark_h = 0.055 if kind == "line" else swatch
            sq = slide.shapes.add_shape(
                geom, int(left + Inches(dx)),
                int(Inches(y + dy + (swatch - mark_h) / 2.0)),
                int(Inches(swatch)), int(Inches(mark_h)))
            if flip:
                xfrm = sq._element.find(".//" + qn("a:xfrm"))
                if xfrm is not None:
                    xfrm.set(_SWATCH_FLIP.get(kind, "flipV"), "1")
            sq.fill.solid()
            sq.fill.fore_color.rgb = col
            if kind == "line":
                sq.line.fill.background()
            else:
                _set_fill_alpha(sq, mark_alpha)
                sq.line.color.rgb = NAVY
                sq.line.width = Pt(0.75)
            sq.shadow.inherit = False
        # How many lines this row needs at its own width, measured in the
        # real font: a legend line that wraps used to overflow its 0.34"
        # box and collide with the row below (Module 6 slide 49, where
        # "Flat fee F* = the entire consumer surplus" needs 4.99" in a
        # 4.46" column).
        avail = (width - Inches(text_dx)) / 914400.0 - 0.06
        n_lines, cur = 1, ""
        for word in text.split():
            trial = (cur + " " + word).strip()
            if _text_w_in(trial, size) <= avail or not cur:
                cur = trial
            else:
                n_lines += 1
                cur = word
        text_h = max(0.34, n_lines * size * 1.22 / 72.0 + 0.06)
        _add_text(slide, int(left + Inches(text_dx)),
                  int(Inches(y + block_h / 2.0 - text_h / 2.0)),
                  int(width - Inches(text_dx)),
                  Inches(text_h), text, size=size, color=NAVY,
                  font="Calibri")
        # a stacked row, or a wrapped line, is taller than one line -- the
        # next row clears whichever is taller
        y += max(pitch, block_h + 0.06, text_h + 0.10)


def _fig_curve_label(slide, fig, xv, yv, label, *, color=NAVY, size=20,
                     bold=True, width=None, clear_of=None, margin=0.04,
                     curve=None):
    ...
    2026-09-09 (Nico): "they should also not overlap with other lines, so
    that they're not covered."  Pass ``clear_of=((x0, y0), (x1, y1))`` --
    the LOGICAL endpoints of the line the label names, or of any line that
    would otherwise run through it -- and the box is pushed along that
    line's normal until it is clear.  Doing it here rather than by nudging
    each anchor means the label stays clear when the curve moves.
    """
    w = width if width is not None else (
        _text_w_in(label, size, bold=bold, italic=True) + 0.08)
    h = 0.32
    left = fig.x(xv)
    top = fig.y(yv) - Inches(0.16)
    if clear_of is not None:
        (lx0, ly0), (lx1, ly1) = clear_of
        ax, ay = fig.x(lx0) / 914400.0, fig.y(ly0) / 914400.0
        bx, by = fig.x(lx1) / 914400.0, fig.y(ly1) / 914400.0
        dx, dy = bx - ax, by - ay
        norm = (dx * dx + dy * dy) ** 0.5
        if norm > 1e-9:
            nx, ny = -dy / norm, dx / norm          # unit normal
            cx = left / 914400.0 + w / 2.0
            cy = top / 914400.0 + h / 2.0
            dist = (cx - ax) * nx + (cy - ay) * ny  # signed distance
            need = (abs(w * nx) + abs(h * ny)) / 2.0 + margin
            if abs(dist) < need:
                # push along the normal, keeping the side it is already on
                sign = 1.0 if dist >= 0 else -1.0
                shift = (need - abs(dist)) * sign
                left = int(left + Inches(nx * shift))
                top = int(top + Inches(ny * shift))
    box = _add_text(slide, int(left), int(top),
                    Inches(w), Inches(h), label, size=size, bold=bold,
                    italic=True, color=color, font="Calibri")
    # paired with its curve by NAME -- see `_fig_line`
    if curve:
        box.name = "sdcurvelab:%s" % curve
    return box
