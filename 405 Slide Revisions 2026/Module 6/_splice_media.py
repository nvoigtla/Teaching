# -*- coding: utf-8 -*-
"""Splice the 3 live PollEverywhere slides into the built Module 6 deck -
the phase-3 media step, RERUNNABLE so _build_Module6.py never freezes.

The source is a SIDECAR, `_handoff_polls_M6.pptx`, carved out of
"Module 6.pptx" by _make_poll_sidecar.ps1 with PowerPoint itself, so every
...
Pipeline:  python _build_Module6.py -> python _splice_media.py [deck]

The engine below is copied verbatim from Module 1/_splice_media.py
(2026-08-27); only the source and the splice map are Module 6's.
"""
import re
import shutil
import sys
...
# target display in "Module 6 - Revised.pptx" -> (sidecar, slide number IN
# THE SIDECAR).  The sidecar keeps the poll slides in their original
# relative order, so its 1-3 are the original deck's 27, 40 and 55.
#
# 2026-09-09: renumbered to 72 / 85 / 96 when the airline-puzzle and
# Lufthansa slides were inserted at 75-76.  These keys are DISPLAY
# numbers, so any insertion before a poll moves them.
#
# All three polls live in the In-Class Examples block: Module 6 is taught
